...
module type Statement = sig
  ...
  val assume : ?loc:location -> term -> t
  ...
  val new_type : ?loc:location -> term -> int -> t
  val type_alias : ?loc:location -> term -> term list -> term -> t
  val type_def : ?loc:location -> term -> term list -> term -> t
  val fun_def : ?loc:location -> term -> term list -> term -> term -> t
